...
| 선택된 행 | 배경 + 좌측 막대 |
| 차트 | `--ez-chart-*` + **범례·직접 라벨·툴팁**. 의미색(success/danger)을 범주에 쓰지 않는다 |

> **차트는 이 원칙의 예외가 아니라 미달 지점이다.** v1.6.1에서 패턴(`aria.decal`)을
> 걷어냈다 — 톤을 흐린다는 판단이었다. 그래서 계열 구분을 색 하나가 지고, 적록
> 색각이상에서 여유가 없다(ΔE 10.6, 안전선 15~20). 대신 **범례를 반드시 남기고,
> 계열이 셋 이상이면 직접 라벨이나 툴팁으로 이름을 읽을 수 있게 한다.**
>
> 색만으로 가르는 조합을 찾는 것은 포기한 게 아니라 **존재하지 않는다** — 2색각은
> 명도·청황 두 축만 봐서, Okabe-Ito·Tableau 10·IBM Carbon까지 시뮬레이션해도
> 적록 ΔE 20을 넘는 6색 조합은 나오지 않는다.
